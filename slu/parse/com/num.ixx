...
namespace slu::parse
{
	...
	constexpr int64_t parseHexInt(AnyInput auto& in, const std::string& str)
	{
		...
		for (const char c : str)
		{
			...
			{
				if (((uint64_t)result & LAST_DIGIT_HEX)
				    != 0) //does it have anything?
				{         // Yes, exit now!
	...
	constexpr ExprType::P128 parseHexU128(
	    AnyInput auto& in, const std::string& str)
	{
	...
	constexpr ExprType::P128 parseU128(
	    AnyInput auto& in, const std::string& str)
	{
	...
	/*
	    ...
	    DecNum ::= ((DigList ["." [DigList]]) | "." DigList) [DecExpSep
	   [NumSign] DigList] ["_" Name]

	    HexNum ::= "0" HexSep ((HexDigList ["." [HexDigList]]) | "." HexDigList)
	   [HexExpSep [NumSign] DigList] ["_" Name]
	...
	*/
	...
	//TODO: parse types
	template<class DataT, bool ALLOW_FLOAT, bool allowSluType = true,
	    AnyInput In>
	inline DataT readNumeralExtra(
	    In& in, const bool hex, const bool decPart, const char firstChar)
	{
		std::string number;
		number += firstChar;
		bool hasDot = decPart;
		...
		while (in)
		{
			...
			const bool digit
			    = (hex && !hasExp) ? isHexDigitChar(c) : isDigitChar(c);
			...
			} else if (requireExpCh)
			{
				throw UnexpectedCharacterError(
				    "Expected a digit or " LUACC_SINGLE_STRING(
				        "+") "/" LUACC_SINGLE_STRING("-") " for exponent of " LC_number
				    + errorLocStr(in));
			} else
				break;
		}
		...
		{
			throw UnexpectedCharacterError(
			    "Found a text character after a " LC_number + errorLocStr(in));
		}
		try
		{
			if (isFloat)
			...
			{
				ExprType::P128 n128
				    = hex ? parseHexU128(in, number) : parseU128(in, number);
		...
		{
			if constexpr (ALLOW_FLOAT)
			{ //Always positive, since negative is
			  // handled by un-ops, in expressions
				return ExprType::F64(INFINITY);
			}
			throwUnexpectedFloat(in, number);
		} catch (const std::exception&)
		{
			throw UnexpectedCharacterError(std::format(LUACC_INVALID
			    "Malformed " LC_number " " LUACC_SINGLE_STRING("{}") "{}",
			    number, errorLocStr(in)));
		}
		throw InternalError("Failed to return from readNumeralExtra");
	}

	export template<class DataT, bool ALLOW_FLOAT = true,
	    bool allowSluType = true, AnyInput In>
	DataT readNumeral(In& in, const char firstChar)
	{
		...
		if (firstChar == '.')
		{
			...
			return readNumeralExtra<DataT, ALLOW_FLOAT, allowSluType>(
			    in, false, true, firstChar);
		}
		if (!in)
