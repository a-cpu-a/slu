...
namespace slu::parse
{
	 ...
	 * @author 31 bit support added by a-cpu-a | credits:
	 * https://github.com/ww898/utf-cpp/blob/master/include/ww898/cp_utf8.hpp
	 * @license MIT
	 *
	...
	/*
	    // NOTE: more ='s can be added, But both sides must have the same amount
	   (closing things inside, with different counts are treated as normal text)
	    // NOTE: NO WHITESPACE BETWEEN CHARS!!!
	    // NOTE: approximate!!

	    StringLiteral ::= "\"" StringLiteralInner "\"" | "'" StringLiteralInner
	   ...
	   "\t" | "\v" | "\"" | "\'" | "\z" | HexEscape | U8Escape | DecEscape

	    HexEscape ::= "\x" HexDigit HexDigit
	    U8Escape ::= "\u{" [[[[[[[Digits_0_To_8] HexDigit] HexDigit] HexDigit]
	   HexDigit] HexDigit] HexDigit] HexDigit "}" DecEscape ::= "\" Digit [Digit
	   [Digit]]
	...
	*/
	export std::string readStringLiteral(
	    AnyInput auto& in, const char firstTypeChar)
	{
		...
		if (firstTypeChar == '"' || firstTypeChar == '\'')
		{
			while (true)
			{
				...
				{
					throw UnexpectedCharacterError(LUACC_INVALID
					    "Unfinished " LC_string ", line ended"
					    + errorLocStr(in));
				}
				...
				if (c == '\\')
				{
					...
					case 'z':
					{
						while (true)
						{
							...
							if (!isSpaceChar(checkChar))
								break;
							if (manageNewlineState<true>(
							        checkChar, nlState, in))
								continue;
					...
					case 'x':
					{
						...
						result
						    += (hexDigit2Num(hex1) << 4) | hexDigit2Num(hex2);
						break;
					}
					case 'u':
					{
						...
						for (uint8_t i = 0; i < 9; i++)
						{
							...
							if (!isHexDigitChar(escapeChar))
							{
								throw UnexpectedCharacterError(std::format(
								    "Expected a hex digit or " LUACC_SINGLE_STRING_INCOL(
								        LUACC_BRACKET,
								        "}}") " for unicode character, but found " LUACC_SINGLE_STRING("{}") "{}",
						...
						if (ch > INT32_MAX)
						{
							throw UnicodeError(std::format(LC_Invalid
							    " unicode character (more than " LUACC_NUM_COL(
							        "31") " bits!) " LUACC_NUMBER
							              "{:x}" LUACC_DEFAULT "{}",
							    ch, errorLocStr(in)));
						}
							...
							if (isDigitChar(ch3))
							{
								...
								if (num > 0xFF)
								{
									throw UnicodeError(std::format(
									    "Unicode " LC_not
									    " allowed in " LUACC_SINGLE_STRING(
									        "\\ddd") "! " LUACC_NUMBER
									                 "{}" LUACC_DEFAULT "{}",
									    num, errorLocStr(in)));
								}
					...
					default:
						throw UnexpectedCharacterError(std::format(
						    "Expected a escape sequence, but found " LUACC_SINGLE_STRING(
						        "{}") "{}",
						    next, errorLocStr(in)));
					}
				} else
