...
namespace slu::parse
{
	    ...
	    requires(AnyCompoundDestr<isLocal, T>)
	{
		...
		if (isValidNameChar(in.peek()) && !checkTextToken(in, "in"))
			ret.name
			    = in.genData.template resolveNewName<isLocal>(readName(in));
		else
	...
	Pat<In, isLocal> readPatPastExpr(In& in, auto&& ty, const bool uncond)
	{
		...
		if (firstChar == '{')
		{
			...
			skipSpace(in);
			if (in.peek() == '|')
				return readFieldsDestr<isLocal,
				    DestrPatType::Fields<In, isLocal>, true>(
				    in, std::move(ty), uncond);

			return readFieldsDestr<isLocal, DestrPatType::List<In, isLocal>,
			    false>(in, std::move(ty), uncond);
		...
		if (!uncond)
		{
			skipSpace(in);
			if (in.peek() == '=')
				return PatType::DestrNameRestrict<In, isLocal>{
				    {nameOrLocal, std::move(ty)},
                    readExpr(in, false)
                };
		}
	...
	Pat<In, isLocal> readPat(In& in, const bool uncond)
	{
		...
		{
			in.skip();
			return PatType::DestrAny<In, isLocal>{
			    in.genData.template resolveNewSynName<isLocal>()};
		}
		...
		{
			return readPatPastExpr<isLocal, false>(
			    in, TypePrefix(std::move(expr.unOps)), uncond);
		}
