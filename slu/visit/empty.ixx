...
#define _Slu_DEF_EMPTY_POST_RAW(_Name, ...) \
	void post##_Name(__VA_ARGS__ itm) {}
#define _Slu_DEF_EMPTY_POST_RAW_LG(_Name, _TY_POST_SYMBOL, ...) \
	_Slu_DEF_EMPTY_POST_RAW(                                    \
	    _Name##Local, __VA_ARGS__<Cfg, true> _TY_POST_SYMBOL);  \
	_Slu_DEF_EMPTY_POST_RAW(                                    \
	    _Name##Global, __VA_ARGS__<Cfg, false> _TY_POST_SYMBOL)
#define _Slu_DEF_EMPTY_POST(_Name, _Ty) _Slu_DEF_EMPTY_POST_RAW(_Name, _Ty&)
#define _Slu_DEF_EMPTY_POST_LG(_Name, _Ty)    \
	_Slu_DEF_EMPTY_POST_RAW_LG(_Name, &, _Ty)
#define _Slu_DEF_EMPTY_POST_UNIT(_Name, _Ty)  \
	_Slu_DEF_EMPTY_POST_RAW(_Name, const _Ty)

#define _Slu_DEF_EMPTY_PRE_RAW(_Name, ...) \
	bool pre##_Name(__VA_ARGS__ itm)       \
	{                                      \
		return false;                      \
	}
#define _Slu_DEF_EMPTY_PRE_RAW_LG(_Name, _TY_POST_SYMBOL, ...)  \
	_Slu_DEF_EMPTY_PRE_RAW(                                     \
	    _Name##Local, __VA_ARGS__<Cfg, true> _TY_POST_SYMBOL);  \
	_Slu_DEF_EMPTY_PRE_RAW(                                     \
	    _Name##Global, __VA_ARGS__<Cfg, false> _TY_POST_SYMBOL)

#define _Slu_DEF_EMPTY_PRE(_Name, ...)          \
	_Slu_DEF_EMPTY_PRE_RAW(_Name, __VA_ARGS__&)
#define _Slu_DEF_EMPTY_PRE_LG(_Name, ...)            \
	_Slu_DEF_EMPTY_PRE_RAW_LG(_Name, &, __VA_ARGS__)
#define _Slu_DEF_EMPTY_PRE_UNIT(_Name, ...)          \
	_Slu_DEF_EMPTY_PRE_RAW(_Name, const __VA_ARGS__)
...
#define _Slu_DEF_EMPTY_PRE_POST(_Name, ...)          \
	_Slu_DEF_EMPTY_PRE_POST_RAW(_Name, __VA_ARGS__&)
#define _Slu_DEF_EMPTY_PRE_POST_LG(_Name, ...)            \
	_Slu_DEF_EMPTY_PRE_POST_RAW_LG(_Name, &, __VA_ARGS__)
#define _Slu_DEF_EMPTY_PRE_POST_UNIT(_Name, ...)          \
	_Slu_DEF_EMPTY_PRE_POST_RAW(_Name, const __VA_ARGS__)

#define _Slu_DEF_EMPTY_AUTO(_Name)                    \
	_Slu_DEF_EMPTY_PRE_POST(_Name, parse::_Name<Cfg>)
#define _Slu_DEF_EMPTY_AUTO_LG(_Name)               \
	_Slu_DEF_EMPTY_PRE_POST_LG(_Name, parse::_Name)

#define _Slu_DEF_EMPTY_SEP_RAW(_Name, _Ty, _ElemTy) \
	void sep##_Name(_Ty list, _ElemTy itm) {}
#define _Slu_DEF_EMPTY_SEP(_Name, _Ty, _ElemTy)                    \
	_Slu_DEF_EMPTY_SEP_RAW(_Name, Slu_esc(_Ty), Slu_esc(_ElemTy&))
...
#define _Slu_DEF_EMPTY_LIST(_Name, _ElemTy)                        \
	_Slu_DEF_EMPTY_PRE_RAW(_Name, std::span<_ElemTy>)              \
		...
		_Slu_DEF_EMPTY_PRE_POST(
		    AnyFuncDeclStat, parse::StatType::FunctionDecl<Cfg>);
		...
		_Slu_DEF_EMPTY_PRE_POST_LG(
		    DestrNameRestrict, parse::PatType::DestrNameRestrict);
		_Slu_DEF_EMPTY_PRE_LG(
		    DestrNameRestrictName, parse::PatType::DestrNameRestrict);
		_Slu_DEF_EMPTY_PRE_LG(
		    DestrNameRestriction, parse::PatType::DestrNameRestrict);
		...
		_Slu_DEF_EMPTY_PRE_UNIT(
		    DestrAny, parse::PatType::DestrAny<Cfg Slu_co true>);
		_Slu_DEF_EMPTY_PRE_UNIT(
		    DestrAny, parse::PatType::DestrAny<Cfg Slu_co false>);
		...
		_Slu_DEF_EMPTY_SEP(DestrListLocal,
		    std::span<parse::Pat<Cfg Slu_co true>>,
		    parse::Pat<Cfg Slu_co true>);
		_Slu_DEF_EMPTY_SEP(DestrListGlobal,
		    std::span<parse::Pat<Cfg Slu_co false>>,
		    parse::Pat<Cfg Slu_co false>);
		...
		_Slu_DEF_EMPTY_SEP(DestrFieldsLocal,
		    std::span<parse::DestrField<Cfg Slu_co true>>,
		    parse::DestrField<Cfg Slu_co true>);
		_Slu_DEF_EMPTY_SEP(DestrFieldsGlobal,
		    std::span<parse::DestrField<Cfg Slu_co false>>,
		    parse::DestrField<Cfg Slu_co false>);
	...
	*/
	...
	concept AnyVisitor = parse::AnyCfgable<T>
	    && std::is_base_of_v<EmptyVisitor<typename T::SettingsT>, T>;
} //namespace slu::visit