...
namespace slu::visit
{
#define Slu_CALL_VISIT_FN_PRE(_Name) if(vi.pre ## _Name (itm))return
#define Slu_CALL_VISIT_FN_SEP(_Name,_i,_vec) if(&_i != &_vec.back()) vi.sep##_Name(_i)
#define Slu_CALL_VISIT_FN_POST(_Name) vi.post##_Name(itm)
