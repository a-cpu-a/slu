...
#define Slu_CALL_VISIT_FN_PRE_LG(_Name)       \
	Slu_CALL_VISIT_FN_PRE_USER_LG(_Name, itm)
#define Slu_CALL_VISIT_FN_PRE_VAR_LG(_Name)   \
	Slu_CALL_VISIT_FN_PRE_USER_LG(_Name, var)
...
#define Slu_CALL_VISIT_FN_POST_USER(_Name, _itm) vi.post##_Name(_itm)
...
#define Slu_CALL_VISIT_FN_POST_VAR(_Name)   \
	Slu_CALL_VISIT_FN_POST_USER(_Name, var)
#define Slu_CALL_VISIT_FN_POST_LG(_Name)       \
	Slu_CALL_VISIT_FN_POST_USER_LG(_Name, itm)
#define Slu_CALL_VISIT_FN_POST_VAR_LG(_Name)   \
	Slu_CALL_VISIT_FN_POST_USER_LG(_Name, var)

	export template<AnyVisitor Vi> void visitString(Vi& vi, std::span<char> itm)
	{
		Slu_CALL_VISIT_FN_PRE(String);
	}
	export template<AnyVisitor Vi> void visitName(Vi& vi, lang::MpItmId& itm)
	{
	...
	export template<AnyVisitor Vi> void visitMp(Vi& vi, lang::MpItmId& itm)
	{
		//TODO
	}
	export template<AnyVisitor Vi>
	...
	void visitDestrSpec(Vi& vi, parse::DestrSpec& itm)
	{
		...
		ezmatch(itm)(
		    varcase(parse::DestrSpecType::Spat&) { visitExpr(vi, var); },
		    varcase(parse::DestrSpecType::Prefix&) { visitUnOps(vi, var); });
	...
	export template<AnyVisitor Vi> void visitTypeExpr(Vi& vi, parse::Expr& itm)
	{
	...
	export template<AnyVisitor Vi> void visitExpr(Vi& vi, parse::Expr& itm)
	{
		Slu_CALL_VISIT_FN_PRE(Expr);
		visitUnOps(vi, itm.unOps);
		visitExprData(vi, itm.data);
		visitPostUnOps(vi,
		    std::span<const ast::PostUnOpType>{
		        itm.postUnOps.data(), itm.postUnOps.size()});
	...
	void visitExprData(Vi& vi, parse::ExprData<Vi>& itm)
	{
		ezmatch(itm)(
		    ...
		    varcase(parse::ExprType::Parens<Vi>&) { visitExpr(vi, *var); },
		    ...
		    varcase(parse::ExprType::PatTypePrefix&) {
			    Slu_panic(
			        "PatTypePrefix is not a valid expression, somehow leaked out of parsing!!!");
		    },


		    varcase(parse::ExprType::Err&) { visitTypeExpr(vi, *var.err); },
		    varcase(const parse::ExprType::Infer) {
			    Slu_CALL_VISIT_FN_PRE_VAR(Infer);
		    },
		    varcase(parse::ExprType::Dyn&) { visitTraitExpr(vi, var.expr); },
		    varcase(parse::ExprType::Impl&) { visitTraitExpr(vi, var.expr); },
		    varcase(parse::ExprType::Struct&) { visitTable(vi, var.fields); },
		    varcase(parse::ExprType::Union&) { visitTable(vi, var.fields); },
	...
	export template<AnyVisitor Vi> void visitArgs(Vi& vi, parse::Args& itm)
	{
		ezmatch(itm)(
	...
	void visitTable(Vi& vi, parse::Table<Vi>& itm)
	{
		...
		for (auto& i : itm)
		{
			ezmatch(i)(
			    ...
			    varcase(parse::FieldType::Expr&) { visitExpr(vi, var); },
			    varcase(parse::FieldType::Expr2Expr&) {
				    visitExpr(vi, var.idx);
				    visitExpr(vi, var.v);
			    },
			    varcase(
			        parse::FieldType::Name2Expr&) { visitExpr(vi, var.v); });
		}
	...
	inline void visitParams(Vi& vi, parse::ParamList& itm)
	{
		...
		{
			ezmatch(i.name)(
			    varcase(parse::LocalId&) { visitNameOrLocal<true>(vi, var); },
			    varcase(lang::MpItmId&) { visitNameOrLocal<false>(vi, var); });
	...
	export template<AnyVisitor Vi> void visitStat(Vi& vi, parse::Stat& itm)
	{
	...
	export template<AnyVisitor Vi> void visitSoe(Vi& vi, parse::Soe<Vi>& itm)
	{
		Slu_CALL_VISIT_FN_PRE(Soe);
		ezmatch(itm)(
		    varcase(parse::SoeType::Block<Vi>&) { visitBlock(vi, var); },
		    varcase(parse::SoeType::Expr&) { visitExpr(vi, *var); });
