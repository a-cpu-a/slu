...
namespace slu::comp
{
	export void poolThread(const CompCfg& cfg, std::atomic_bool& shouldExit,
	    std::condition_variable& cv, std::condition_variable& cvMain,
	    ...
	    a_cpu_a::Mutex<std::vector<CompTask>>& tasks)
	{
		...
		TaskHandleState state{.s = &mlirState,
		...
		//mlir::populateFinalizeMemRefToLLVMConversionPatterns(state.typeConverter,
		//patterns);
		//mlir::cf::populateControlFlowToLLVMConversionPatterns(state.typeConverter,
		//patterns);
		//mlir::index::populateIndexToLLVMConversionPatterns(state.typeConverter,
		//patterns);
		//mlir::arith::populateArithToLLVMConversionPatterns(state.typeConverter,
		//patterns);
		//mlir::populateFuncToLLVMConversionPatterns(state.typeConverter,
		//patterns);
		...
		mlirState.indexLay
		    = mlir::DataLayoutEntryAttr::get(mlirState.opBuilder.getIndexType(),
		...
		// Add passes in order
		mlirState.pm.addPass(
		    mlir::memref::createExpandStridedMetadataPass()); // memref → memref
		                                                      // + affine +
		                                                      // arith
		mlirState.pm.addPass(
		    mlir::createFinalizeMemRefToLLVMConversionPass()); // memref → LLVM
		mlirState.pm.addPass(
		    mlir::createLowerAffinePass()); // Affine → SCF + arith
		mlirState.pm.addPass(mlir::createSCFToControlFlowPass()); // SCF → CF
		mlirState.pm.addPass(
		    mlir::createConvertControlFlowToLLVMPass()); // cf → LLVM
		mlirState.pm.addPass(
		    mlir::createArithToLLVMConversionPass()); // arith → LLVM
		mlirState.pm.addPass(
		    mlir::createConvertIndexToLLVMPass()); // index → LLVM
		mlirState.pm.addPass(
		    mlir::createConvertFuncToLLVMPass()); // func → LLVM
		...
		while (true)
		{
			...
			if (shouldExit)
				return;
			if (tasks.v.empty())
				cv.wait(taskLock, [&shouldExit, &tasks]() {
					return shouldExit || !tasks.v.empty();
				});
			if (shouldExit)
				return;
			if (tasks.v.empty())
			...
			if (isCompleterThread)
			{
				...
				tasks.v.pop_back();
				taskLock.unlock(); //only thread with this stuff, so no need to
				                   //keep a lock!
			}
			...
			//Complete it...
			handleTask(cfg, shouldExit, state,
			    stackCopy.has_value() ? nullptr : &taskLock, task);
