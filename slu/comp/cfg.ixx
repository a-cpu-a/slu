...
namespace slu::comp
{
	...
	// The compiler wont emit `../`, so there is no need to sanitize them for
	// that (except by you in rootPaths) Your paths must not contain `\` If one
	// of your callbacks is thread-unsafe, add some mutex, because all of them
	// will be called by many threads.
	//
	...
	{
		// If it would need to call to some dll, or external code, it is likely
		// not safe
		...
		using GetItemListFn
		    = std::vector<int> (*)(const lang::ModPathView& path);
	...
	export struct TmpFile
	{
		...
		{}
		constexpr TmpFile(DestroyFn destroy) : destroy(destroy) {};
	...
	{
		// you need to add a newline yourself
		...
		using MkTmpFileFn
		    = TmpFile (*)(std::optional<std::span<const uint8_t>> contents);
		MkTmpFileFn mkTmpFilePtr;

		using GetFileContentsFn = std::optional<std::vector<uint8_t>> (*)(
		...
		//file paths inside a path, return a empty list if
		// the path is invalid or doesnt exist
		using GetFileListFn
		    = std::vector<std::string> (*)(const std::string_view path);
		GetFileListFn getFileListPtr;
		GetFileListFn getFileListRecPtr; //recursive version

		std::span<const CompPlugin> plugins;

		// All of them must have some crate.
		std::span<const std::string> rootPaths;
