...
namespace slu::comp::mico
{
	...
	export struct ConvData : CommonConvData
	{
		...
		GlobElemTy::Fn* addElem(const lang::MpItmId name,
		    mlir::func::FuncOp func, std::string_view abi)
		{
			if (name.mp.id >= mp2Elements.size())
		...
		GlobElemTy::Alias* addElem(
		    const lang::MpItmId name, const lang::MpItmId realName)
		{
			if (name.mp.id >= mp2Elements.size())
				mp2Elements.resize(name.mp.id + 1);
			auto& mp = mp2Elements[name.mp.id];
	...
	//Forward declare!
	mlir::Value convExpr(
	    ConvData& conv, ast::Position place, const parse::ExprDataV<true>& itm);
	...
	mlir::Location convPos(ConvData& conv, ast::Position p)
	{
		return mlir::FileLineColLoc::get(
		    &conv.context, conv.filePath, (uint32_t)p.line,
	...
	struct ViewOrStr
	{
		...
		{
			return std::visit(
			    [](const auto& v) { return std::string_view{v}; }, val);
		}
	};
	ViewOrStr mangleFuncName(
	    ConvData& conv, const std::string_view abi, lang::MpItmId name)
	{
		if (abi == "C")
	...
	mlir::Type tryConvBuiltinType(ConvData& conv, const std::string_view abi,
	    const lang::MpItmId& name, const bool reffed)
	{
		if (name == mpc::STD_STR)
		{
			if (!reffed)
				throw std::runtime_error(
				    "Unimplemented type expression: std::str, without ref (mlir conversion, reffed expected)");
			...
			return mlir::MemRefType::get(
			    {mlir::ShapedType::kDynamic}, i8Type, {}, 0);
		}
		if (name == mpc::STD_I32)
		{
		...
		throw std::runtime_error("Unimplemented type expression: "
		    + std::string(name.asSv(conv.sharedDb)) + " (mlir conversion)");
	}
	mlir::Type convTypeHack(
	    ConvData& conv, const std::string_view abi, const parse::Expr& expr)
	{
		return ezmatch(expr.data)(
		    varcase(const auto&)->mlir::Type {
			    throw std::runtime_error("Unimplemented type expression idx("
			        + std::to_string(expr.data.index())
			        + ") (mlir conversion)");
		    },
		    ...
		    varcase(const parse::ExprType::SelfCall&) {
			    if (var.method
			        != conv.sharedDb.getItm({"std", "ops", "Ref", "ref"}))
				    throw std::runtime_error("Unimplemented type expression: "
				        + std::string(var.method.asSv(conv.sharedDb))
				        + " (mlir conversion)");

			    const auto& selfName
			        = std::get<parse::ExprType::GlobalV<true>>(var.v->data);
		    ...
		    varcase(const parse::ExprType::Call&)->mlir::Type {
			    auto name
			        = std::get<parse::ExprType::GlobalV<true>>(var.v->data);
			    throw std::runtime_error("Unimplemented type expression: "
			        + std::string(name.asSv(conv.sharedDb))
			        + " (mlir conversion)");

			    //const auto& expArgs =
			    //std::get<parse::ArgsType::ExprList>(var.args); const auto&
			    //firstArgExpr = expArgs.front(); const auto& firstArg =
			    //std::get<parse::ExprType::GlobalV<true>>(firstArgExpr.data);
			    //
			    //return tryConvBuiltinType(conv, abi, firstArg, true);
		    });
	}

	mlir::Type convType(ConvData& conv, const parse::ResolvedType& itm,
	    const std::string_view abi)
	{
		...
		const bool isRefOrPtrToSlice = ezmatch(itm.base)(
		    ...
		    varcase(const parse::RawTypeKind::Ref&) {
			    if (std::holds_alternative<parse::RawTypeKind::Slice>(
			            var->elem.base))
			    {
				    //slicePtr =
				    //&std::get<parse::RawTypeKind::Slice>(var->elem.base);
				    return true;
			    }
			    return false;
		    },
		...
		if (itmUsized)
		{
			if (cAbi)
				throw std::runtime_error(
				    "Found unsized type in C ABI (mlir conversion)");
			elemType = builder.getI8Type(); // `? x i8`, aka byte memref
		} else
		{
			if (cAbi && itm.size == parse::TYPE_RES_PTR_SIZE
			    && !isRefOrPtrToSlice // Ensure it is not &[]Zst (it stores []
			                          // size, thats it, cuz elems are 0 bits &
			                          // stride is not useful)
			    && isRefOrPtr)
	    ...
	    ConvData& conv, ast::Position place, const parse::Any64BitInt auto itm)
	{
		...
		return mlir::arith::ConstantOp::create(builder, convPos(conv, place),
		    i64Type, mlir::IntegerAttr::get(i64Type, (int64_t)itm));
	}
	inline mlir::Value convAny128(
	    ConvData& conv, ast::Position place, const parse::Any128BitInt auto itm)
	{
		mlir::OpBuilder& builder = conv.builder;
		auto i128Type = builder.getIntegerType(128);
		llvm::APInt apVal(128, llvm::ArrayRef{itm.lo, itm.hi});
		return mlir::arith::ConstantOp::create(builder, convPos(conv, place),
		    i128Type, mlir::IntegerAttr::get(i128Type, apVal));
	}
	inline mlir::Value convExpr(
	    ConvData& conv, ast::Position place, const parse::ExprDataV<true>& itm)
	{
		...
		return ezmatch(itm)(
		    ...
		    varcase(const auto&)->mlir::Value {
			    ...
			    throw std::runtime_error("Unimplemented expression type idx("
			        + std::to_string(itm.index()) + ") (mlir conversion)");
		    },
		    varcase(const parse::ExprType::I64) {
			    return convAny64(conv, place, var);
		    },
		    varcase(const parse::ExprType::U64) {
			    return convAny64(conv, place, var);
		    },
		    varcase(const parse::ExprType::P128) {
			    return convAny128(conv, place, var);
		    },
		    ...
		    varcase(const parse::ExprType::FieldV<true>&) {
			    ...
			    if (var.field
			        == conv.sharedDb.getPoolStr("__convHack__refSlice_ptr"sv))
			    {
				    //Hack: take out ptr.

				    mlir::Type idx3Type = mlir::MemRefType::get(
				        {3}, builder.getIndexType(), {}, 0);
				    ...
				    mlir::Value lForm
				        = mlir::UnrealizedConversionCastOp::create(builder, loc,
				            mlir::TypeRange{rawMemref}, mlir::ValueRange{base})
				              .getResult(0);
				    mlir::Value idx3Form
				        = mlir::UnrealizedConversionCastOp::create(builder, loc,
				            mlir::TypeRange{idx3Type}, mlir::ValueRange{lForm})
				              .getResult(0);

				    mlir::Value c0
				        = mlir::arith::ConstantIndexOp::create(builder, loc, 0);
				    mlir::Value idxPtr = mlir::memref::LoadOp::create(builder,
				        loc, idx3Form, mlir::ValueRange{c0}, false, 0ULL);

				    mlir::Value intPtr = mlir::index::CastUOp::create(
				        builder, loc, builder.getI64Type(), idxPtr);

				    return mlir::LLVM::IntToPtrOp::create(builder, loc,
				        mlir::LLVM::LLVMPointerType::get(&conv.context), intPtr)
				        .getRes();
			    }
			    //TODO sub;
			    //basicly (.x == memref subview?) (.y.x().x == multiple stuff)
			    throw std::runtime_error(
			        "TODO: Field expr's are unimplemented (mlir conversion)");
		    },
		    varcase(const parse::ExprType::String&)->mlir::Value {
			    auto i8Type = builder.getI8Type();
			    auto i64Type = builder.getI64Type();
			    auto llvmPtrType = mlir::LLVM::LLVMPointerType::get(mc);
			    auto strType = mlir::MemRefType::get(
			        {(int64_t)var.v.size()}, i8Type, {}, 0);

			    constexpr uint32_t REF_SLICE_SIZE
			        = parse::TYPE_RES_PTR_SIZE + parse::TYPE_RES_SIZE_SIZE * 2;
			    auto ptrNsizeX2Int = builder.getIntegerType(REF_SLICE_SIZE);
			    auto refSliceBytesType = mlir::MemRefType::get(
			        {(REF_SLICE_SIZE + 7) / 8}, i8Type, {}, 0);
			    auto refSliceType
			        = mlir::MemRefType::get({1}, ptrNsizeX2Int, {}, 0);
			    auto idx3Type
			        = mlir::MemRefType::get({3}, builder.getIndexType(), {}, 0);

			    auto tensorType = mlir::RankedTensorType::get(
			        {(int64_t)var.v.size()}, i8Type);
			    auto denseStr = mlir::DenseElementsAttr::get(tensorType,
			        llvm::ArrayRef{(const int8_t*)var.v.data(), var.v.size()});
			    ...
			    TmpName strName = mkTmpName(conv);
			    {
				    ...
				    mlir::memref::GlobalOp::create(builder, loc, strName.sref(),
				        /*sym_visibility=*/conv.privVis, strType, denseStr,
				        /*constant=*/true,
				        /*alignment=*/builder.getIntegerAttr(i64Type, 1));
			    }

			    // %str = memref.get_global @strName : memref<14xi8>
			    auto globalStr = mlir::memref::GetGlobalOp::create(
			        builder, loc, strType, strName.sref());

			    mlir::Value refSliceAlloc = mlir::memref::AllocaOp::create(
			        builder, loc, refSliceType);

			    //Reinterpret as 3xindex.
			    mlir::Value cv0 = mlir::arith::ConstantIntOp::create(
			        builder, loc, i64Type, 0);
			    mlir::Value cv3 = mlir::arith::ConstantIntOp::create(
			        builder, loc, i64Type, 3);
			    mlir::Value c0
			        = mlir::arith::ConstantIndexOp::create(builder, loc, 0);
			    mlir::Value c1
			    ...
			    mlir::Value ptr
			        = mlir::memref::ExtractAlignedPointerAsIndexOp::create(
			            builder, loc, globalStr);
			    auto data = mlir::memref::ExtractStridedMetadataOp::create(
			        builder, loc, globalStr);

			    //mlir::Value idx3Form =
			    //slu_dial::ReinterpretMemRefOp::create(builder,
			    //	loc,idx3Type,refSliceAlloc
			    //);
			    mlir::Value idx3Form = mlir::UnrealizedConversionCastOp::create(
			        builder, loc, mlir::TypeRange{idx3Type},
			        mlir::ValueRange{refSliceAlloc})
			                               .getResult(0);


			    // TODO: check if offset matters. NOTE: wont matter in this
			    // specific use case.

			    mlir::memref::StoreOp::create(
			        builder, loc, ptr, idx3Form, mlir::ValueRange{c0});
			    mlir::memref::StoreOp::create(builder, loc, data.getSizes()[0],
			        idx3Form, mlir::ValueRange{c1});
			    mlir::memref::StoreOp::create(builder, loc,
			        data.getStrides()[0], idx3Form, mlir::ValueRange{c2});

			    //mlir::Value sz64 =
			    //mlir::arith::IndexCastUIOp::create(builder,loc, i64Type,
			    //data.getSizes()[0]); mlir::Value sz192 =
			    //mlir::arith::ExtUIOp::create(builder,loc, ptrNsizeX2Int,
			    //sz64);
			    //
			    //mlir::Value str64 =
			    //mlir::arith::IndexCastUIOp::create(builder,loc, i64Type,
			    //data.getStrides()[0]); mlir::Value str192 =
			    //mlir::arith::ExtUIOp::create(builder,loc, ptrNsizeX2Int,
			    //str64);
			    //
			    //mlir::Value ptr64 =
			    //mlir::arith::IndexCastUIOp::create(builder,loc, i64Type, ptr);
			    //mlir::Value ptr192 = mlir::arith::ExtUIOp::create(builder,loc,
			    //ptrNsizeX2Int, ptr64);
			    //
			    //mlir::Value sizeShifted =
			    //mlir::arith::ShLIOp::create(builder,loc, sz192, c64);
			    //mlir::Value offsetShifted =
			    //mlir::arith::ShLIOp::create(builder,loc, str192, c128);
			    //
			    //// OR them together
			    //mlir::Value part1 = mlir::arith::OrIOp::create(builder,loc,
			    //ptr192, sizeShifted); mlir::Value packed =
			    //mlir::arith::OrIOp::create(builder,loc, part1, offsetShifted);
			    //
			    //return packed;
		    ...
		    varcase(const AnyInvalidExpression auto&)->mlir::Value {
			    throw std::runtime_error("Invalid expression type idx("
			        + std::to_string(itm.index()) + ") (mlir conversion)");
		    });
	}
	...
	inline GlobElemTy::Fn* getOrDeclFn(ConvData& conv, lang::MpItmId name,
	    ast::Position place, const parse::ItmType::Fn* funcItmOrNull)
	{
		auto* mc = &conv.context;
		mlir::OpBuilder& builder = conv.builder;
		...
		if (funcItmOrNull == nullptr)
		{
			...
			if (realName != name)
			{
				GlobElem* funcInfo = conv.getElement(realName);
				if (funcInfo != nullptr)
					return &std::get<GlobElemTy::Fn>(
					    *funcInfo); //todo: require it to be fn once other stuff
					                //is added.
			}
			funcItmOrNull
			    = &parse::getItm<parse::ItmType::Fn>(conv.sharedDb, realName);
		}
		...
		mlir::func::FuncOp funcOp = mlir::func::FuncOp::create(builder,
		    ...
		    getExportAttr(conv, false), nullptr, nullptr, false);
		if (name != realName)
	...
	inline std::optional<mlir::Value> convSoeOrBlock(
	    ConvData& conv, const parse::SoeOrBlockV<true>& itm)
	{
		return ezmatch(itm)(
		    varcase(const parse::SoeType::Expr&)->std::optional<mlir::Value> {
			    return convExpr(conv, var->place, var->data);
		    },
		    varcase(const parse::SoeType::BlockV<true>&)
	...
	inline void convStat(ConvData& conv, const parse::Stat& itm)
	{
		...
		ezmatch(itm.data)(
		    ...
		    varcase(const auto&) {
			    throw std::runtime_error("Unimplemented statement type idx("
			        + std::to_string(itm.data.index()) + ") (mlir conversion)");
		    },
		    varcase(const parse::StatType::CanonicLocal&) {
			    mlir::Value alloc
			        = convExpr(conv, var.value.place, var.value.data);
		    ...
		    varcase(const parse::StatType::RepeatUntilV<true>&) {
			    ...
			    mlir::Value one = mlir::arith::ConstantIntOp::create(
			        builder, loc, i1Type, 1);

			    auto whileOp = mlir::scf::WhileOp::create(builder, loc,
			        mlir::TypeRange{i1Type}, mlir::ValueRange{one});
			    ...
			    mlir::scf::ConditionOp::create(
			        builder, loc, condArg, mlir::ValueRange{condArg});
			    ...
			    mlir::scf::YieldOp::create(builder, convPos(conv, var.bl.end),
			        mlir::ValueRange{continueLoop});

			    builder.setInsertionPointAfter(whileOp);
		    },
		    varcase(const parse::StatType::WhileV<true>&) {
			    auto whileOp = mlir::scf::WhileOp::create(builder,
			        convPos(conv, itm.place), mlir::TypeRange{},
			        mlir::ValueRange{});

			    builder.setInsertionPointToStart(whileOp.getBeforeBody());
			    mlir::scf::ConditionOp::create(builder,
			        convPos(conv, var.cond.place),
			        convExpr(conv, var.cond.place, var.cond.data),
			        mlir::ValueRange{});

			    builder.setInsertionPointToStart(whileOp.getAfterBody());
			    if (convBlock(conv, var.bl))
				    mlir::scf::YieldOp::create(
				        builder, convPos(conv, var.bl.end));
		    ...
		    varcase(const parse::StatType::BlockV<true>&) {
			    auto scopeOp = mlir::memref::AllocaScopeOp::create(
			        builder, convPos(conv, itm.place), mlir::TypeRange{});
		    ...
		    varcase(const parse::StatType::IfCondV<true>&) {
			    ...
			    yieldBlocks.reserve(
			        var.elseIfs.size() + (var.elseBlock.has_value() ? 1 : 0));
			    size_t elIfIdx = 0;
			    ...
			    do
			    {
				    ...
				    if (elIfIdx == 0)
				    {
					    ...
					    hasMore
					        = var.elseBlock.has_value() || !var.elseIfs.empty();
				    } else
				    {
					    ...
					    hasMore = var.elseBlock.has_value()
					        || (var.elseIfs.size() > elIfIdx);
				    }

				    auto op = mlir::scf::IfOp::create(builder, loc,
				        mlir::TypeRange{},
				    ...
				    if (ret.has_value())
				    {
					    if (*ret == nullptr)
						    mlir::scf::YieldOp::create(
						        builder, convPos(conv, itm.place));
					    else
						    mlir::scf::YieldOp::create(
						        builder, convPos(conv, itm.place), *ret);
				    }
			    ...
			    for (const auto i : yieldBlocks)
			    {
				    ...
				    mlir::scf::YieldOp::create(builder,
				        convPos(conv, itm.place)); //TODO: return something.
			    }
		    ...
		    varcase(const parse::StatType::AssignV<true>&) {
			    if (var.vars.size() != 1 || var.exprs.size() != 1)
				    throw std::runtime_error(
				        "Unimplemented assign conv, vers.size or var.exprs != 1");


			    mlir::Value memRef = convExpr(conv, itm.place, var.vars[0]);
			    const mlir::Location loc = convPos(conv, itm.place);

			    mlir::Value expr
			        = convExpr(conv, var.exprs[0].place, var.exprs[0].data);
			    //Expr is a memref, so copy the bits/bytes.
			    mlir::memref::CopyOp::create(builder, loc, expr, memRef);

			    //auto zeroIndex =
			    //mlir::arith::ConstantIndexOp::create(builder,loc, 0);
			    //mlir::memref::StoreOp::create(builder,loc, expr, memRef,
			    //mlir::ValueRange{ zeroIndex }, false);
		    },
		    varcase(const parse::StatType::Call&) {
			    auto name
			        = std::get<parse::ExprType::GlobalV<true>>(var.v->data);
			    GlobElemTy::Fn* funcInfo
			        = getOrDeclFn(conv, name, itm.place, nullptr);
			    ...
			    mlir::func::CallOp::create(builder, convPos(conv, itm.place),
			        funcInfo->func.getResultTypes(),
			        mlir::SymbolRefAttr::get(mc, funcInfo->func.getSymName()),
			        llvm::ArrayRef<mlir::Value>{args});
		    },
		    varcase(const parse::StatType::ConstV<true>&) {
			    ...
			    auto i64Type = builder.getI64Type();
			    auto strType = mlir::MemRefType::get(
			        {(int64_t)str.size()}, i8Type, {}, 0);

			    auto tensorType = mlir::RankedTensorType::get(
			        {(int64_t)str.size()}, i8Type);
			    auto denseStr = mlir::DenseElementsAttr::get(tensorType,
			        llvm::ArrayRef{(const int8_t*)str.data(), str.size()});
		    ...
		    varcase(const parse::StatType::Fn&) {
			    ...
			    const parse::ItmType::Fn& funcItm
			        = parse::getItm<parse::ItmType::Fn>(
			            conv.sharedDb, var.name);
			    const bool cAbi = funcItm.abi == "C"sv;

			    GlobElemTy::Fn* funcInfo
			        = getOrDeclFn(conv, var.name, itm.place, &funcItm);
			    if (funcItm.exported)
			    ...
			    for (size_t i = 0; i < funcItm.argLocals.size(); i++)
			    {
				    ...
				    if (cAbi)
				    { //C ABI: arguments are passed by value, so we need to
					  //allocate a memref for them.
					    auto memrefType
					        = mlir::MemRefType::get({1}, val.getType());
					    mlir::Value alloc = mlir::memref::AllocaOp::create(
					        builder, loc, memrefType);
					    mlir::Value index0
					        = mlir::arith::ConstantIndexOp::create(
					            builder, loc, 0);
					    mlir::memref::StoreOp::create(
					        builder, loc, val, alloc, mlir::ValueRange{index0});
					    val = alloc;
				    }
				    localTop.values[id.v] = {val, &funcItm.args[i]};
			    }

			    if (convBlock(conv, var.func.block))
				    mlir::func::ReturnOp::create(
				        builder, convPos(conv, var.func.block.end));
