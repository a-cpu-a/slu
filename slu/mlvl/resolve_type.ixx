...
namespace slu::mlvl
{
	export parse::ResolvedType resolveTypeExpr(
	    parse::BasicMpDb mpDb, parse::Expr&& type);

	void handleTypeExprField(parse::BasicMpDb mpDb, size_t& nameIdx,
	    parse::FieldV<true>& field, auto& res)
	{
		ezmatch(std::move(field))(
		    ...
		    varcase(parse::FieldType::Name2Expr&&) {
			    ...
			    res.fields.emplace_back(
			        resolveTypeExpr(mpDb, std::move(var.v)));
		    },
		    varcase(parse::FieldType::Expr2Expr&&) {
			    throw std::runtime_error(
			        "FieldType::Expr2ExprV type resolution: TODO not implemented: jit the expression");
		    },
		    varcase(const parse::FieldType::NONE) {
			    throw std::runtime_error(
			        "FieldType::NONE should not exist in struct/union type expression.");
		    });
	}

	parse::ResolvedType resolveStructType(
	    parse::BasicMpDb mpDb, parse::TableV<true>&& itm)
	{
		...
		for (parse::FieldV<true>& field : itm)
		{
			...
			//TODO: fix field offsets changing depending on named field order,
			//and tuple field order between them too.
			...
			if (!resField.isSized())
				fieldOffset
				    = parse::ResolvedType::UNSIZED_MARK; //Following fields will
				                                         //only have offsets
				                                         //known at runtime.
		}

		return parse::ResolvedType{.base = parse::RawTypeKind::Struct(&res),
		    .size = fieldOffset,
		    .alignmentData = maxAlign};
	}
	parse::ResolvedType resolveTypeExpr(
	    parse::BasicMpDb mpDb, parse::Expr&& type)
	{
		parse::ResolvedType resTy = ezmatch(std::move(type.data))(
		    ...
		    varcase(const parse::ExprType::U64) {
			    return parse::ResolvedType::getConstType(
			        parse::RawTypeKind::Uint64{var});
		    },
		    varcase(const parse::ExprType::P128) {
			    return parse::ResolvedType::getConstType(
			        parse::RawTypeKind::Pos128{var.lo, var.hi});
		    },
		    varcase(const parse::ExprType::M128) {
			    return parse::ResolvedType::getConstType(
			        parse::RawTypeKind::Neg128{var.lo, var.hi});
		    },
		    ...
		    varcase(parse::ExprType::Impl&&)->parse::ResolvedType {
			    throw std::runtime_error(
			        "TODO: resolve Impl type expressions.");
		    },
		    ...
		    varcase(parse::ExprType::SelfCall&&)->parse::ResolvedType {
			    ...
			    if (expArgs.size() != 0)
				    throw std::runtime_error(
				        "TODO: self call type expressions with >0 arg.");

			    parse::ResolvedType rt
			        = resolveTypeExpr(mpDb, std::move(*var.v));

			    if (var.method
			        == mpDb.data->getItm({"std", "ops", "MarkMut", "markMut"}))
			    {
				    if (rt.hasMut)
					    throw std::runtime_error(
					        "Invalid type expression: used 'mut' on already marked as mutable type.");
				    rt.hasMut = true;
				    return rt;
			    }
			    ast::UnOpType op;
			    if (var.method
			        == mpDb.data->getItm({"std", "ops", "Ref", "ref"}))
				    op = ast::UnOpType::REF;
			    //else if (var.method == mpDb.data->getItm({
			    //"std","ops","Ptr","ptr" })) 	op = ast::UnOpType::PTR;
			    else if (var.method
			        == mpDb.data->getItm({"std", "ops", "Slicify", "slicify"}))
				    op = ast::UnOpType::SLICIFY;
			    else
				    throw std::runtime_error("Unimplemented type expression: "
				        + std::string(var.method.asSv(mpDb))
				        + " (type resolution)");
			    ...
			    {
				    //size_t sz = zst ? 0 : (parse::TYPE_RES_PTR_SIZE +
				    //parse::TYPE_RES_SIZE_SIZE * 2 * rt.outerSliceDims);
				    //TODO: fix slice refs.
			    }
			    return parse::ResolvedType{
		    ...
		    [&](const parse::ExprType::GlobalV<true> name)
		        -> parse::ResolvedType {
			    if (name == mpc::STD_STR)
			    ...
			    if (name == mpc::STD_I32)
			    {
				    return parse::ResolvedType{
				        .base
				        = parse::RawTypeKind::Range64{INT32_MIN, INT32_MAX},
				        .size = 32,
			    ...
			    throw std::runtime_error(
			        "TODO: resolve complex lim-prefix-expr type expressions.");
		    },
		    ...
		    varcase(parse::ExprType::Union&&)->parse::ResolvedType {
			    ...
			    for (parse::FieldV<true>& field : var.fields)
			    {
				    ...
				    if (maxSize == parse::ResolvedType::INCOMPLETE_MARK)
					    continue; //If any field is incomplete, then union is
					              //incomplete.

				    const parse::ResolvedType& resField = res.fields.back();
				    maxAlign
				        = std::max(maxAlign, (uint8_t)resField.alignmentData);
				    ...
				    if (maxSize == parse::ResolvedType::UNSIZED_MARK)
					    continue; //Field is complete, but still the union will
					              //be unsized.
		    ...
		    varcase(parse::ExprType::IfCondV<true>&&)->parse::ResolvedType {
			    //TODO
			    //return
			    //parse::ResolvedType::getConstType(parse::RawTypeKind::Uint128{
			    //var.lo,var.hi });
			    throw std::runtime_error("TODO: resolve if type expressions.");
		    },


#define Slu_INVALID_EXPR_CASE(_MSG, ...)                      \
	varcase(__VA_ARGS__&&)->parse::ResolvedType               \
	{                                                         \
		throw std::runtime_error(                             \
		    "Invalid slu " _MSG ", index:" #__VA_ARGS__ "."); \
	}
		    ...
		    varcase(parse::ExprType::MultiOpV<true>&&)->parse::ResolvedType {
			    throw std::runtime_error(
			        "Multi-op type expressions are ment to be desuagared before type resolution.");
		    });
