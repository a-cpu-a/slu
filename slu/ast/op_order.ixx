...
namespace slu::ast
{
	export enum class Assoc : uint8_t
	{
		LEFT,
		RIGHT
	};
	...
	export constexpr Assoc associativity(ast::BinOpType op)
	{
		...
		case ast::BinOpType::REP: return Assoc::RIGHT;
		default:                  return Assoc::LEFT;
		}
	}

	export enum class OpKind : uint8_t
	{
		BinOp,
		UnOp,
		PostUnOp,
	...
	constexpr void consumeUnOps(std::vector<MultiOpOrderEntry>& unOps,
	    const auto& item, const size_t itemIdx, ExprUnOpsEntry& entry,
	    const bool onLeftSide, const uint8_t minPrecedence)
	{
		if (!entry.used)
		{
			unOps.insert(unOps.end(),
			    MultiOpOrderEntry{.index = itemIdx, .kind = OpKind::Expr});
			entry.used = true;
		}
		...
		while (pSuf != item.postUnOps.cend() || pPre != item.unOps.crend())
		{
			...
			const uint8_t prePrec
			    = preAlive ? calcPrecedence(pPre, item.unOps.crend()) : 0;
			const uint8_t sufPrec
			    = sufAlive ? calcPrecedence(pSuf, item.postUnOps.cend()) : 0;

			if (sufAlive && (!preAlive || sufPrec > prePrec))
			{
				//Once its not allowed once, it never will be, ever again.
				allowLeft
				    = allowLeft && (onLeftSide || sufPrec > minPrecedence);
				if (allowLeft)
				{
					unOps.push_back({itemIdx, entry.sufConsumed++,
					    OpKind::PostUnOp, sufPrec, Assoc::LEFT});
				}
				pSuf++;
			} else
			{
				allowRight
				    = allowRight && (!onLeftSide || prePrec > minPrecedence);
				if (allowRight)
				{
					unOps.push_back({itemIdx, entry.preConsumed++, OpKind::UnOp,
					    prePrec, Assoc::RIGHT});
				}
				pPre++;
			}
	...
	export constexpr std::vector<MultiOpOrderEntry> multiOpOrder(const auto& m)
	{
		...
		When sorting, binop to binop order is always the same, even when
		removing all unary ops. This means you can sort the un ops after
		bin-ops.
		...
		If you have a un op with a higher precedence than a bin op, it will
		always be applied before even if there are lower un ops before it. With
		equal precedences, the un ops are applied after. Any un ops in between a
		bin op are applied before the bin op.

		un op precedences between other un ops only matters if they are on
		different sides of a expr. This means that un ops will expand outwards
		from the expression, going to the highest precedence un op, until
		matching the precedence of a bin op.

		The effective precedence of a un op is the highest of any that are
		applied after, and itself.
		(*-0) -> negative takes the max of (-,*) sees that * has more, so copies
		it.
		*/
		...
		for (size_t i = 0; i < m.extra.size(); ++i)
		{
			...
			ops.push_back(
			    {i + 1, 0, OpKind::BinOp, precedence(bin), associativity(bin)});
		}

		std::sort(ops.begin(), ops.end(),
		    [](const MultiOpOrderEntry& a, const MultiOpOrderEntry& b) {
			    if (a.precedence != b.precedence)
				    return a.precedence > b.precedence;

			    return (a.assoc == Assoc::LEFT) ? a.index < b.index
			                                    : a.index > b.index;
		    });
		...
		for (size_t i = 0; i < ops.size(); i++)
		{
			...
			const auto& left
			    = (e.index == 1) ? *m.first : m.extra[e.index - 2].second;
			...
			const size_t item1Idx
			    = lAssoc ? e.index - 1 : e.index; // -1, cuz 1 is first expr
			...
			consumeUnOps(
			    unOps[i], item2, item2Idx, ent2, !lAssoc, e.precedence);
		}
		...
		consumeUnOps(unOpsLast, m.extra.back().second, m.extra.size(),
		    exprUnOps.back(), true, 0xFF);
		...
		std::move(
		    unOpsLast.begin(), unOpsLast.end(), std::back_inserter(opsRes));

		Slu_assertOp(
		    opsRes.size(), ==, ops.size() + unOpCount + 1 + m.extra.size());

		return opsRes;
	}

	/// @returns the order of unary operations as a vector of bools, where true
	/// means post-unary operation
	export constexpr std::vector<bool> unaryOpOrder(const parse::Expr& expr)
	{
		...
		{
			if (precedence(expr.unOps[unOpIdx])
			    >= precedence(expr.postUnOps.at(postUnOpIdx)))
			{
				res[i] = false; // UnOp
				unOpIdx--;
				if (unOpIdx == SIZE_MAX)
