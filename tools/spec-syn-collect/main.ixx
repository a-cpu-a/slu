/*
    Spec syntax collector, extracts the syntax from the slu spec.
...
export module tools_slu.spec_syn_collect.main;
...
std::string extract_and_merge_ebnf_blocks(const std::string& directory = "in")
{
	...
	for (const auto& entry : fs::recursive_directory_iterator(directory))
	{
		...
		while ((pos = sv.find(start_token, pos)) != std::string_view::npos)
		{
			...
			std::string block
			    = std::string(sv.substr(block_start, i - block_start - 1));
			...
			while (std::getline(lines, line))
			{
				...
				} else if (std::regex_search(line, comment_line))
				{
					accumulating = false;
					linesData.push_back(
					    {.aboveSpacing = linesAbove, .lines = {line}});
					linesAbove = 0;
				...
				{
					auto& lines
					    = linesData[rules[current_rule].lineDataIdx].lines;
...
export int main()
{
	const auto x = extract_and_merge_ebnf_blocks("spec/");
	std::ofstream myfile;
	myfile.open("out/collected-spec-syn.txt");
	myfile << x;
	myfile.close();
	return 0;
}
